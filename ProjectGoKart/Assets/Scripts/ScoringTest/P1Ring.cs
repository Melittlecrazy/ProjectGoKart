...
public class P1Ring : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (!p1activated)
        {
            ...
            {
                checkpoints.GetComponent<Scoring>().player1score = checkpoints.GetComponent<Scoring>().player1score + 1f;
                kart.material.color = Color.red;
                p1activated = true;
            }
        }
            if (other.gameObject.tag == "Enemy" && other.gameObject.GetComponent<HasBall>().hasBall == true && p1activated == true)
            {
                if (checkpoints.GetComponent<Scoring>().player2score == 1) checkpoints.GetComponent<Scoring>().player2score = checkpoints.GetComponent<Scoring>().player2score - 0;
                kart.material.color = Color.white;
                p2activated = false;
                checkpoints.GetComponent<Scoring>().player2score = checkpoints.GetComponent<Scoring>().player2score - .5f;
                //checkpoints.GetComponent<Scoring>().player2score = checkpoints.GetComponent<Scoring>().player2score + .5f;
            }
    }
